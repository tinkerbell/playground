...
6. Watch for the provisioner to complete
   ...
   wid=$(docker-compose exec tink-cli tink workflow get --no-headers | awk '/^\|/ {print $2}'); docker-compose exec tink-cli watch -n1 "tink workflow events ${wid}; tink workflow state ${wid}"
   ```
